...
def create_chat_server_config(tmpdir: Path, model_copied: bool = False) -> dict[str, str]:
    ...
    with open(app_confdir / 'ProjectLLM.ini', 'w') as conf:
    ...
    if model_copied:
        app_data_dir = tmpdir / 'share' / 'vm' / 'AIModel'
...
def prepare_chat_server(model_copied: bool = False) -> Iterator[dict[str, str]]:
    ...
    with tempfile.TemporaryDirectory(prefix='ProjectLLM-test') as td:
        tmpdir = Path(td)
...
def test_with_models_temperature(chat_server_with_model: None) -> None:
    """Fixed by nomic-ai/ProjectLLM#3202."""
    data = {
        'model': 'Llama 3.2 1B Instruct',
        'prompt': 'The quick brown fox',
        'temperature': 0.5,
