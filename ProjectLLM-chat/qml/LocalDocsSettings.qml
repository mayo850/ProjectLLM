...
MySettingsTab {
    ...
    contentItem: ColumnLayout {
        ...
        RowLayout {
            ...
            MyTextField {
                ...
                onEditingFinished: {
                    ...
                    /* Blacklist common unsupported file extensions. for now only support plain text and PDFs, and although this will
                     * reject binary data, i don't want to waste time trying to index files that we don't support. */
                    exts = exts.filter(e => ![
        ...
        RowLayout {
            MySettingsLabel {
                text: qsTr("Use VM Embed API")
                helpText: qsTr("Embed documents using the fast API (if implemented - jeremy?) instead of a private local model. Requires restart.")
            }
        ...
        RowLayout {
            MySettingsLabel {
                id: apiKeyLabel
                text: qsTr("VM API Key")
                helpText: qsTr('API key to use for VM Embed. Requires restart.')
        ...
        MySettingsLabel {
            ...
            helpText: qsTr("Values too large may cause localdocs failure, extremely slow responses or failure to respond at all. Roughly speaking, the {N chars x N snippets} are added to the model's context window.")
            onLinkActivated: function(link) { Qt.openUrlExternally(link) }
        }
