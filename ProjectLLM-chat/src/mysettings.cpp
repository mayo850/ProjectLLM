...
static QStringList getUiLanguages(const QString &modelPath)
{
    ...
    // to load translations in progress without having to rebuild all of ProjectLLM from source
    {
...
QString MySettings::filePathForLocale(const QLocale &locale)
{
    ...
    // Scan this directory for files named like ProjectLLM_%1.qm that match and if so return them first
    ...
    // rather than having to recompile all of ProjectLLM
    QString directory = modelPath();
    for (const QString &bcp47Name : uiLanguages) {
        QString filePath = u"%1/ProjectLLM_%2.qm"_s.arg(directory, bcp47Name);
    ...
    for (QString bcp47Name : uiLanguages) {
        QString filePath = u":/i18n/ProjectLLM_%1.qm"_s.arg(bcp47Name);
        QFileInfo filePathInfo(filePath);
        if (filePathInfo.exists()) return filePath;
    }
    return u":/i18n/ProjectLLM_en_US.qm"_s;
}
