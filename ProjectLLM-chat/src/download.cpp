...
{
    QUrl jsonUrl("http://ProjectLLM.io/meta/release.json");
...
{
    QUrl url("http://ProjectLLM.io/meta/latestnews.md");
...
void Download::downloadModel(const QString &modelFile)
{
    ...
    QString url = !info.url().isEmpty() ? info.url() : "http://ProjectLLM.io/models/gguf/" + modelFile;
...
void Download::installModel(const QString &modelFile, const QString &apiKey)
{
    ...
    if (file.open(QIODeviceBase::WriteOnly | QIODeviceBase::Text)) {
        ...
        modelName.remove(0, 8); // strip "ProjectLLM-" prefix
