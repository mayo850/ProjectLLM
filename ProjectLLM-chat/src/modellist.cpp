...
#include <ProjectLLM-backend/llmodel.h>
...
static const QStringList FILENAME_BLACKLIST { u"ProjectLLM-nomic-embed-text-v1.rmodel"_s };
...
QString ModelList::compatibleModelFilename(QUrl baseUrl, QString modelName) {
    ...
    return QString(u"ProjectLLM-%1-capi.rmodel"_s).arg(hash);
}
...
void ModelList::updateOldRemoteModels(const QString &path)
{
    ...
    while (it.hasNext()) {
        ...
        QFile newfile(u"%1/ProjectLLM-%2.rmodel"_s.arg(info.dir().path(), modelname));
...
void ModelList::processModelDirectory(const QString &path)
{
    ...
    while (it.hasNext()) {
        ...
        for (const QString &id : modelsById) {
            ...
            if (isCompatibleApi) {
                // The data will be saved to "ProjectLLM.ini".
                data.append({ NameRole, name });
                // The description is hard-coded into "ProjectLLM.ini" due to performance issue.
...
void ModelList::updateModelsFromJson()
{
    ...
    QUrl jsonUrl(u"file://%1/dev/large_language_models/ProjectLLM/ProjectLLM-chat/metadata/%2"_s.arg(QDir::homePath(), modelsJsonFname));
#else
    QUrl jsonUrl(u"http://ProjectLLM.io/models/%1"_s.arg(modelsJsonFname));
#endif
...
void ModelList::parseModelsJsonFile(const QByteArray &jsonData, bool save)
{
...

    {
        ...
        const QString id = modelName;
        const QString modelFilename = "ProjectLLM-gpt-3.5-turbo.rmodel";
        if (contains(modelFilename))
            changeId(modelFilename, id);
        if (!contains(id))
            addModel(id);
...

    {
        ...
        const QString id = modelName;
        const QString modelFilename = "ProjectLLM-gpt-4.rmodel";
        if (contains(modelFilename))
            changeId(modelFilename, id);
        if (!contains(id))
            addModel(id);
...

    {
        ...
        const QString id = modelName;
        const QString modelFilename = "ProjectLLM-mistral-tiny.rmodel";
        if (contains(modelFilename))
            changeId(modelFilename, id);
        if (!contains(id))
            addModel(id);
    ...
    }
    {
        ...
        const QString id = modelName;
        const QString modelFilename = "ProjectLLM-mistral-small.rmodel";
        if (contains(modelFilename))
            changeId(modelFilename, id);
        if (!contains(id))
            addModel(id);
...

    {
        ...
        const QString id = modelName;
        const QString modelFilename = "ProjectLLM-mistral-medium.rmodel";
        if (contains(modelFilename))
            changeId(modelFilename, id);
        if (!contains(id))
            addModel(id);
