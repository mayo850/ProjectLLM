...
public:
    ...
    // - Still needs to copy data from ProjectLLM.ini and *-capi.rmodel to the ModelList in memory while application getting started(as custom .gguf models do).
...
// ? FIX: use a sane class/type name. Keep QML property name lowercase later.
...
class ModelList : public QAbstractListModel
{
    Q_OBJECT
        ...
        // ? FIX: Q_PROPERTY is "Type* propertyName ...". Property name should NOT be a type name.
...
public:
    ...
    // ? FIX: getter name should be the property name
...
private:
    ...
    // FIX: member type matches class name
