...
from ProjectLLM import ProjectLLM, Embed4All
import time
import pytest
...
def test_inference_long_falcon():
    model = ProjectLLM(model_name='ProjectLLM-falcon-q4_0.gguf')
    do_long_input(model)


def test_inference_long_llama_7b():
    model = ProjectLLM(model_name="mistral-7b-openorca.Q4_0.gguf")
    do_long_input(model)


def test_inference_long_llama_13b():
    model = ProjectLLM(model_name='nous-hermes-llama2-13b.Q4_0.gguf')
    do_long_input(model)


def test_inference_long_mpt():
    model = ProjectLLM(model_name='mpt-7b-chat-q4_0.gguf')
    do_long_input(model)


def test_inference_long_replit():
    model = ProjectLLM(model_name='replit-code-v1_5-3b-q4_0.gguf')
    do_long_input(model)
...
def test_download_model(tmp_path: Path):
    ...
    old_default_dir = ProjectLLM.DEFAULT_MODEL_DIRECTORY
    ProjectLLM.DEFAULT_MODEL_DIRECTORY = tmp_path  # temporary pytest directory to ensure a download happens
    try:
        model = ProjectLLM(model_name='ggml-all-MiniLM-L6-v2-f16.bin')
        ...
        ProjectLLM.DEFAULT_MODEL_DIRECTORY = old_default_dir
