    ...
    if platform.system() == "Darwin" and platform.processor() == "i386":
        ...
        if p.returncode == 0 and p.stdout.strip() == "1":
            raise RuntimeError(textwrap.dedent("""\
                Running ProjectLLM under Rosetta is not supported due to CPU feature requirements.
                Please install ProjectLLM in an environment that uses a native ARM64 Python interpreter.
            """).strip())
...
MODEL_LIB_PATH = importlib_resources.files("ProjectLLM") / "llmodel_DO_NOT_MODIFY" / "build"
...
class LLModel:
    """
    Base class and universal wrapper for ProjectLLM language models
    ...
    def __init__(self, model_path: str, n_ctx: int, ngl: int, backend: str):
        ...
        if model is None:
            ...
            if (
                ...
                print('WARNING: CUDA runtime libraries not found. Try `pip install "ProjectLLM[cuda]"`\n', file=sys.stderr)
