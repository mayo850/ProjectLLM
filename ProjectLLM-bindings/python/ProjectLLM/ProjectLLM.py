"""
Python only API for running all ProjectLLM models.
"""
...
# TODO: move to config
DEFAULT_MODEL_DIRECTORY = Path.home() / ".cache" / "ProjectLLM"
...
class Embed4All:
    """
    Python class that handles embeddings for ProjectLLM.
    """
    ...
    def __init__(self, model_name: str | None = None, *, n_threads: int | None = None, device: str | None = None, **kwargs: Any):
        ...
        Args:
            n_threads: number of CPU threads used by ProjectLLM. Default is None, then the number of threads are determined automatically.
            device: The processing unit on which the embedding model will run. See the `ProjectLLM` constructor for more info.
            kwargs: Remaining keyword arguments are passed to the `ProjectLLM` constructor.
        """
        if model_name is None:
            model_name = "all-MiniLM-L6-v2.gguf2.f16.gguf"
        self.ProjectLLM = ProjectLLM(model_name, n_threads=n_threads, device=device, **kwargs)
    ...
    def embed(
        ...
        result = self.ProjectLLM.model.generate_embeddings(text, prefix, dimensionality, do_mean, atlas, cancel_cb)
        return result if return_dict else result["embeddings"]


class ProjectLLM:
    """
    Python class that handles instantiation, downloading, generation and chat with ProjectLLM models.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            model_name: Name of ProjectLLM or custom model. Including ".gguf" file extension is optional but encouraged.
            model_path: Path to directory containing model file or, if file does not exist, where to download model.
                Default is None, in which case models will be stored in `~/.cache/ProjectLLM/`.
            model_type: Model architecture. This argument currently does not have any functionality and is just used as
                descriptive identifier for user. Default is None.
            allow_download: Allow API to download models from ProjectLLM.io. Default is True.
            n_threads: number of CPU threads used by ProjectLLM. Default is None, then the number of threads are determined automatically.
            device: The processing unit on which the ProjectLLM model will run. It can be set to:
                ...
                - A specific device name from the list returned by `ProjectLLM.list_gpus()`.
                Default is Metal on ARM64 macOS, "cpu" otherwise.

                Note: If a selected GPU device does not have sufficient RAM to accommodate the model, an error will be thrown, and the ProjectLLM instance will be rendered invalid. It's advised to ensure the device has enough memory before initiating the model.
    ...
    def list_models() -> list[ConfigType]:
        """
        Fetch model list from https://ProjectLLM.io/models/models3.json.

        Returns:
            Model list in JSON format.
        """
        resp = requests.get("https://ProjectLLM.io/models/models3.json")
    ...
    def retrieve_model(
        ...
        Args:
            ...
            model_path: Path to find model. Default is None in which case path is set to
                ~/.cache/ProjectLLM/.
            allow_download: Allow API to download model from ProjectLLM.io. Default is True.
    ...
    def download_model(
        ...
        """
        Download model from ProjectLLM.io.
        ...
        # Download model
        if url is None:
            url = f"https://ProjectLLM.io/models/gguf/{model_filename}"
    ...
    def generate(
        ...
        *,
        ...
        """
        Generate outputs from any ProjectLLM model.
    ...
    def chat_session(
        ...
        """
        Context manager to hold an inference optimized chat session with a ProjectLLM model.
