# ProjectLLM API Server

ProjectLLM provides a local API server that allows you to run LLMs over an HTTP API. 

## Key Features

- **Local Execution**: Run models on your own hardware for privacy and offline use.
- **LocalDocs Integration**: Run the API with relevant text snippets provided to your LLM from a [LocalDocs collection](../ProjectLLM_desktop/localdocs.md).
...
1. Open the ProjectLLM Chat Desktop Application.
...
!!! note "Example ProjectLLM API calls"
...
1. Open the Chats view in the ProjectLLM application.
...
(Note: LocalDocs can currently only be activated through the ProjectLLM UI, not via the API itself).
