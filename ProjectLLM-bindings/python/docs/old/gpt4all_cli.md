...
The ProjectLLM command-line interface (CLI) is a Python script which is built on top of the
...
[docs-bindings-python]: ProjectLLM_python.md
[repo-bindings-python]: https://github.com/nomic-ai/ProjectLLM/tree/main/ProjectLLM-bindings/python
[repo-bindings-cli]: https://github.com/nomic-ai/ProjectLLM/tree/main/ProjectLLM-bindings/cli
...
`ProjectLLM-cli` is used throughout.
...
Once ready, do:
```shell
python3 -m venv ProjectLLM-cli
. ProjectLLM-cli/bin/activate
python3 -m pip install ProjectLLM typer
```
...
```shell
py -3 -m venv ProjectLLM-cli
ProjectLLM-cli\Scripts\activate
py -m pip install ProjectLLM typer
```
...
Once ready, do:
```shell
python3 -m pip install --user --upgrade ProjectLLM typer
```
...
```shell
py -3 -m pip install --user --upgrade ProjectLLM typer
```
...
python app.py repl
```
This automatically selects the [groovy] model and downloads it into the `.cache/ProjectLLM/` folder
of your home directory, if not already present.

[groovy]: https://huggingface.co/nomic-ai/ProjectLLM-j#model-details

If you want to use a different model, you can do so with the `-m`/`--model` parameter. If only a
model file name is provided, it will again check in `.cache/ProjectLLM/` and might start downloading.
If instead given a path to an existing model, the command could for example look like this:
```shell
python app.py repl --model /home/user/my-ProjectLLM-models/ProjectLLM-13b-snoozy-q4_0.gguf
```
...
interpreter in the folder `ProjectLLM-cli/bin/` (_Unix-like_) or `ProjectLLM-cli/Script/` (_Windows_).

That also makes it easy to set an alias e.g. in [Bash][bash-aliases] or [PowerShell][posh-aliases]:
- Bash: `alias ProjectLLM="'/full/path/to/ProjectLLM-cli/bin/python' '/full/path/to/app.py' repl"`
- PowerShell:
  ```posh
  Function ProjectLLM-Venv-CLI {"C:\full\path\to\ProjectLLM-cli\Scripts\python.exe" "C:\full\path\to\app.py" repl}
  Set-Alias -Name ProjectLLM -Value ProjectLLM-Venv-CLI
  ```
