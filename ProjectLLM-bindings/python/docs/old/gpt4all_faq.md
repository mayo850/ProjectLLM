...
This is a breaking change that renders all previous models (including the ones that ProjectLLM uses) inoperative with newer versions of llama.cpp since
that change.
...
or `allowDownload=true` (default), a model is automatically downloaded into `.cache/ProjectLLM/` in the user's home folder,
...
<https://ProjectLLM.io/> website instead. Scroll down to 'Model Explorer' and pick your preferred model.
