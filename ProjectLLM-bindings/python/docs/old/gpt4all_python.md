# ProjectLLM Python Generation API
The `ProjectLLM` python package provides bindings to our C/C++ model backend libraries.
The source code and local build instructions can be found [here](https://github.com/nomic-ai/ProjectLLM/tree/main/ProjectLLM-bindings/python).
...
- Instantiate `ProjectLLM`,  which is the primary public API to your large language model (LLM).
- Automatically download the given model to `~/.cache/ProjectLLM/` if not already present.

Read further to see how to chat with this model.


### Chatting with ProjectLLM
...
=== "ProjectLLM Example"
    ``` py
    model = ProjectLLM(model_name='orca-mini-3b-gguf2-q4_0.gguf')
...
When using ProjectLLM models in the `chat_session()` context:
...
- Each prompt passed to `generate()` is wrapped in the appropriate prompt template. If you pass `allow_download=False`
  to ProjectLLM or are using a model that is not from the official models list, you must pass a prompt template using the
...
[models3.json]: https://github.com/nomic-ai/ProjectLLM/blob/main/ProjectLLM-chat/metadata/models3.json


### Streaming Generations
To interact with ProjectLLM responses as the model generates, use the `streaming=True` flag during generation.

=== "ProjectLLM Streaming Example"
    ``` py
    from ProjectLLM import ProjectLLM
    model = ProjectLLM("orca-mini-3b-gguf2-q4_0.gguf")
    tokens = []
...
::: ProjectLLM.ProjectLLM.ProjectLLM.generate
...
The model folder can be set with the `model_path` parameter when creating a `ProjectLLM` instance. The example below is
is the same as if it weren't provided; that is, `~/.cache/ProjectLLM/` is the default folder.

``` py
from pathlib import Path
from ProjectLLM import ProjectLLM
model = ProjectLLM(model_name='orca-mini-3b-gguf2-q4_0.gguf', model_path=Path.home() / '.cache' / 'ProjectLLM')
```
...
=== "macOS"
    ...
    model_path = Path.home() / 'Library' / 'Application Support' / 'nomic.ai' / 'ProjectLLM'
    model = ProjectLLM(model_name, model_path)
    ```
=== "Windows"
    ``` py
    from pathlib import Path
    ...
    import os
    model_name = 'orca-mini-3b-gguf2-q4_0.gguf'
    model_path = Path(os.environ['LOCALAPPDATA']) / 'nomic.ai' / 'ProjectLLM'
    model = ProjectLLM(model_name, model_path)
    ```
=== "Linux"
    ``` py
    ...
    model_name = 'orca-mini-3b-gguf2-q4_0.gguf'
    model_path = Path.home() / '.local' / 'share' / 'nomic.ai' / 'ProjectLLM'
    model = ProjectLLM(model_name, model_path)
    ```
...
``` py
from pathlib import Path
from ProjectLLM import ProjectLLM, ProjectLLM
ProjectLLM.DEFAULT_MODEL_DIRECTORY = Path.home() / 'my' / 'models-directory'
model = ProjectLLM('orca-mini-3b-gguf2-q4_0.gguf')
```
...
=== "ProjectLLM Custom Session Templates Example"
    ``` py
    from ProjectLLM import ProjectLLM
    model = ProjectLLM('wizardlm-13b-v1.2.Q4_0.gguf')
...
To prevent ProjectLLM from accessing online resources, instantiate it with `allow_download=False`. When using this flag,
there will be no default system prompt by default, and you must specify the prompt template yourself.

You can retrieve a model's default system prompt and prompt template with an online instance of ProjectLLM:
...
``` py
from ProjectLLM import ProjectLLM
model = ProjectLLM('orca-mini-3b-gguf2-q4_0.gguf', allow_download=False)
...
=== "ProjectLLM Custom Stop Callback"
    ``` py
    from ProjectLLM import ProjectLLM
    model = ProjectLLM('orca-mini-3b-gguf2-q4_0.gguf')
...
## API Documentation
::: ProjectLLM.ProjectLLM.ProjectLLM
