...
The [ProjectLLM Chat Client](https://ProjectLLM.io) lets you easily interact with any local large language model.

It is optimized to run 7-13B parameter LLMs on the CPU's of any computer running OSX/Windows/Linux.

## Running LLMs on CPU
The ProjectLLM Chat UI supports models from all newer versions of `llama.cpp` with `GGUF` models including the `Mistral`, `LLaMA2`, `LLaMA`, `OpenLLaMa`, `Falcon`, `MPT`, `Replit`, `Starcoder`, and `Bert` architectures

ProjectLLM maintains an official list of recommended models located in [models3.json](https://github.com/nomic-ai/ProjectLLM/blob/main/ProjectLLM-chat/metadata/models3.json). You can pull request new models to it and if accepted they will show up in the official download dialog.

#### Sideloading any GGUF model
If a model is compatible with the ProjectLLM-backend, you can sideload it into ProjectLLM Chat by:
...
2. Identifying your ProjectLLM model downloads folder. This is the path listed at the bottom of the downloads dialog.
3. Placing your downloaded model inside ProjectLLM's model downloads folder.
4. Restarting your ProjectLLM app. Your model should appear in the model selection list.

## Plugins
ProjectLLM Chat Plugins allow you to expand the capabilities of Local LLMs.

### LocalDocs Plugin (Chat With Your Data)
LocalDocs is a ProjectLLM feature that allows you to chat with your local files and data.
...
<p align="center">
  <img width="70%" src="https://github.com/nomic-ai/ProjectLLM/assets/10168/fe5dd3c0-b3cc-4701-98d3-0280dfbcf26f">
</p>

#### Enabling LocalDocs
1. Install the latest version of ProjectLLM Chat from [ProjectLLM Website](https://ProjectLLM.io).
...
ProjectLLM Chat comes with a built-in server mode allowing you to programmatically interact
...
`4891` (the reverse of 1984). You can enable the webserver via `ProjectLLM Chat > Settings > Enable web server`.
...
# model = "gpt-3.5-turbo"
#model = "mpt-7b-chat"
model = "ProjectLLM-j-v1.3-groovy"
