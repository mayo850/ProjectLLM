# ProjectLLM Node.js API

Native Node.js LLM bindings for all.

```sh
yarn add ProjectLLM@latest

npm install ProjectLLM@latest

pnpm install ProjectLLM@latest
...
*   ProjectLLM nodejs bindings created by [jacoobes](https://github.com/jacoobes), [limez](https://github.com/iimez) and the [nomic ai community](https://home.nomic.ai), for all to use.
...
```js
import { LLModel, createCompletion, DEFAULT_DIRECTORY, DEFAULT_LIBRARIES_DIRECTORY, loadModel } from '../src/ProjectLLM.js'
...
```js
import { loadModel, createEmbedding } from '../src/ProjectLLM.js'
...
```js
import { loadModel, createCompletion } from "../src/ProjectLLM.js";
...
```js
import gpt from "../src/ProjectLLM.js";
...
*   MingW works as well to build the ProjectLLM-backend. **HOWEVER**, this package works only with MSVC built dlls.
...
*   python 3
*   On Windows and Linux, building ProjectLLM requires the complete Vulkan SDK. You may download it from here: https://vulkan.lunarg.com/sdk/home
*   macOS users do not need Vulkan, as ProjectLLM will use Metal instead.

### Build (from source)

```sh
git clone https://github.com/nomic-ai/ProjectLLM.git
cd ProjectLLM-bindings/typescript
```
...
*   llama.cpp git submodule for ProjectLLM can be possibly absent. If this is the case, make sure to run in llama.cpp parent directory
...
*   \[x] publish to npm under alpha tag `ProjectLLM@alpha`
...
*   "gpu name": Model will run on the GPU that matches the name if it's available.
    Note: If a GPU device lacks sufficient RAM to accommodate the model, an error will be thrown, and the ProjectLLM
...
By default this will download a model from the official ProjectLLM website, if a model is not present at given path.
...
From python api:
models will be stored in (homedir)/.cache/ProjectLLM/\`
...
```javascript
const download = downloadModel('ggml-ProjectLLM-j-v1.3-groovy.bin')
...
Remote download url. Defaults to `https://ProjectLLM.io/models/gguf/<modelName>`
