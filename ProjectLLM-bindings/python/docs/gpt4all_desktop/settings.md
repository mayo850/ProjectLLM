...
!!! note "General Application Settings"
    ...
    | **Device** | Device that will run your models. Options are `Auto` (ProjectLLM chooses), `Metal` (Apple Silicon M1+), `CPU`, and `GPU` | `Auto` |
    ...
    | **Download Path** | Select a destination on your device to save downloaded models | Windows: `C:\Users\{username}\AppData\Local\nomic.ai\ProjectLLM`<br><br>Mac: `/Users/{username}/Library/Application Support/nomic.ai/ProjectLLM/`<br><br>Linux: `/home/{username}/.local/share/nomic.ai/ProjectLLM` |
    | **Enable Datalake** | Opt-in to sharing interactions with ProjectLLM community (**anonymous** and **optional**) | Off |
...
!!! note "Advanced Application Settings"
    ...
    | **Enable Local Server** | Allow any application on your device to use ProjectLLM via an OpenAI-compatible ProjectLLM API | Off |
...
!!! note "General LocalDocs Settings"
    ...
    | **Embeddings Device** | Device that will run embedding models. Options are `Auto` (ProjectLLM chooses), `Metal` (Apple Silicon M1+), `CPU`, and `GPU` | `Auto` |
