...
## What changed in ProjectLLM v3.5?
ProjectLLM v3.5 overhauled the chat template system. There are three crucial differences:
...
If you are using any chat templates or system messages that had been added or altered from the default before upgrading to ProjectLLM v3.5 or newer, these will no longer work. See below for how to solve common errors you may see after upgrading.
...
This is the result of attempting to use an old-style template (possibly from a previous version) in ProjectLLM 3.5+.
...
1. Back up your chat template by copying it safely to a text file and saving it. In the next step, it will be removed from ProjectLLM.
...
Otherwise, you have either typed something correctly, or the model comes with a template that is incompatible with ProjectLLM. See [the below section][creating] on creating chat templates and make sure that everything is correct. When in doubt, ask on the [Discord].


## Error: No chat template configured.
This may appear for models that are not from the official model list and do not include a chat template. Older versions of ProjectLLM picked a poor default in this case. You will get much better results if you follow the steps to [find][finding] or [create][creating] a chat template for your model.
...
ProjectLLM also supports the special variables `bos_token`, `eos_token`, and `add_generation_prompt`. See the [HuggingFace docs] for what those do.
...
# Advanced: What are ProjectLLM v1 templates?
ProjectLLM supports its own template syntax, which is nonstandard but provides complete control over the way LocalDocs sources and file attachments are inserted into the conversation. These templates begin with `{# ProjectLLM v1 #}` and look similar to the example below.

For standard templates, ProjectLLM combines the user message, sources, and attachments into the `content` field. For ProjectLLM v1 templates, this is not done, so they must be used directly in the template for those features to work correctly.
