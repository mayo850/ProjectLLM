...
The ProjectLLM CLI is a self-contained script based on the `ProjectLLM` and `typer` packages. It offers a
...
Welcome to the ProjectLLM CLI! Version {VERSION}
...
def repl(
    ...
    ProjectLLM_instance = ProjectLLM(model, device=device)

    # if threads are passed, set them
    if n_threads is not None:
        num_threads = ProjectLLM_instance.model.thread_count()
        print(f"\nAdjusted: {num_threads} →", end="")

        # set number of threads
        ProjectLLM_instance.model.set_thread_count(n_threads)

        num_threads = ProjectLLM_instance.model.thread_count()
        print(f" {num_threads} threads", end="", flush=True)
    else:
        print(f"\nUsing {ProjectLLM_instance.model.thread_count()} threads", end="")

    print(CLI_START_MESSAGE)

    use_new_loop = False
    try:
        version = importlib.metadata.version('ProjectLLM')
    ...
    if use_new_loop:
        _new_loop(ProjectLLM_instance)
    else:
        _old_loop(ProjectLLM_instance)


def _old_loop(ProjectLLM_instance):
    while True:
        message = input(" ⇢  ")
        ...
        full_response = ProjectLLM_instance.chat_completion(
            MESSAGES,
...
def _new_loop(ProjectLLM_instance):
    with ProjectLLM_instance.chat_session():
        while True:
            message = input(" ⇢  ")
            ...
            # execute chat completion and ignore the full response since 
            # we are outputting it incrementally
            response_generator = ProjectLLM_instance.generate(
                message,
                # preferential kwargs for chat ux
                max_tokens=200,
                temp=0.9,
            ...
            response.close()
            ProjectLLM_instance.current_chat_session.append(response_message)
...
def version():
    ...
    print(f"ProjectLLM-cli v{VERSION}")
